  ...
  before do 
    class ErbBinding
      def initialize(hash)
        ...
        def tags_path opt; raise NotImplementedError end
      end
    end
    expect(bind).to receive(:form_for).with(:tagging).and_yield(builder)
    expect(bind).to receive(:tags_path).with(format: :json){ :tags_path } 
    ...
    expect(builder).to receive(:text_field).
      with(:tag_id, data:{url: :tags_path}){ "select_tag_id" }
